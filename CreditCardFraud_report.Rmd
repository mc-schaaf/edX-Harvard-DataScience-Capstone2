...
Our understanding of shopping is changing rapidly. In the past years online shopping has become more and more omnipresent. E-commerce sales worldwide exhibit staggering growth rates of 20 percent annualy. However, the total sales are estimated to grow by [only 5 percent annualy](quote)^[quote]. 
...
Following the definition pf PCA, features ``V1`` up to ``V28`` are uncorrelated. Some small correlation with ``Time`` (first feature) and ``Amount`` (last feature) exists. However, this small a magnitude of covariance is not a problem for logistic regression or the other used machine learning algorithms.

```{r FE_covariance_matrix}
rafalib::mypar(1,1)
...
```{r DV_scatter, results = "hide", out.width = "33%", eval = FALSE}
...
```{r DV_density, results = "hide", out.width = "25%", eval = FALSE}
...
Remember that the goal is to have perfect sensitivity for all values of specificity. This means a line that shoots straight into the upper right corner and stays at a sensitivity of 1 would be ideal. The logistic regression seems to be able to capture about 80% of the fraudulent transactions without generating large amounts of false positive cases. The last 20%, however, seem not detectable with ordinary logistic regression.
...
```{r AM1_backwards_eval, echo=FALSE}
...
```{r AM1_undersampling_applied, warning = FALSE, results = "hide"}
...
```{r AM2_PCR, warning=FALSE, message=FALSE, results = "hide"}
...
```{r AM2_elasticnet, warning = FALSE, echo = FALSE}
...
```{r AM2_elasticnet_ROC, echo=FALSE}
...
As our primary gaol is an conpairson and not very fine parameter tuning, we will only use 10-fold cross validation and a low-resoluting tune grid.
...
Now we can apply various different machine learning algorithms with very little effort and compaire their results:

```{r AM3_do_caret, cache=TRUE}
...
for (i in c(1:25)){
  ...
  tmp <- map_df(ps, function(p){
    y_hat <- ifelse(p_Fraud <= p, "Legitimate", "Fraud") %>% factor(levels = levels(data_train$Class))
    SS <- caret::confusionMatrix(y_hat, BT_data$Class, positive = "Fraud")$byClass[c("Sensitivity", "Specificity", "F1")]
    AC <- caret::confusionMatrix(y_hat, BT_data$Class, positive = "Fraud")$overall["Accuracy"]
    list(method = paste0("Bootstrap Sample ", i), cutoff = p, sensitivity = SS[1], specificity = SS[2], F1 = SS[3], accuracy = AC[1])
  })
  # save the evaluation
  if (i == 1){
    BT_results <- tmp
...
the second question is a question of weighting up the costs of false-positive cases. This remains to be decided by the financial institution incorporating the fraud detection. 
